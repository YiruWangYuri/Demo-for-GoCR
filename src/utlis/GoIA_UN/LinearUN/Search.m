...
%     draw_u = animatedline;   
    ...
    while(1)
        ...
        iter = iter+1;
        for nk = branchnum + 1 : branchnum + num_
            branchL_pq.push(nk, -branchQueue( end-1, nk));
        end
        branchnum = branchnum + num_;    

    
        best_L = -branchL_pq.top_value(); % Take a peek at the top of the queue. This should be 200
        index = branchL_pq.top(); % Get the ID of the top value. This should be 3
        branchL_pq.pop(); % Pop (remove) the top
